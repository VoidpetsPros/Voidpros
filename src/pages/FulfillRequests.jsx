...
import { useMyFulfilledRequests } from "../hooks/useMyFulfilledRequests";
...
export default function FulfillRequests() {
  ...
  const { builds: fulfilledForMe, loading: fulfilledLoading } = useMyFulfilledRequests(user?.id);
  ...
  return (
    <div style={{ padding: "24px 24px 60px", maxWidth: 640, margin: "0 auto" }}>
      {!fulfilledLoading && fulfilledForMe.length > 0 && (
        <div style={{ marginBottom: 32 }}>
          <p style={{ fontSize: 11, color: GOLD, textTransform: "uppercase", letterSpacing: 1, marginBottom: 12, display: "flex", alignItems: "center", gap: 6 }}>
            <PartyPopper size={13} /> Your request{fulfilledForMe.length > 1 ? "s were" : " was"} fulfilled
          </p>
          {fulfilledForMe.map((b) => (
            <BuildCard key={b.id} build={b} pets={pets} items={items} ownedPets={ownedPets} ownedItemCounts={ownedItems} fullMatch={true} />
          ))}
        </div>
      )}
          ...
          <div key={r.id} style={{ background: PANEL, border: `1px solid ${LINE}`, borderRadius: 12, padding: 18, marginBottom: 14 }}>
            ...
            <div style={{ display: "flex", flexWrap: "wrap", gap: 8, marginBottom: 16 }}>
              {r.pets.map((pid) => {
                ...
                if (!p) return null;
                return (
                  <span key={pid} style={{ display: "flex", alignItems: "center", gap: 6, fontSize: 12.5, padding: "4px 10px 4px 4px", borderRadius: 20, background: "rgba(108,86,201,0.1)", color: VIOLET, border: "1px solid rgba(108,86,201,0.3)" }}>
              ...
              {Object.entries(r.items).map(([iid, count]) => {
                ...
                if (!it) return null;
                return (
                  <span key={iid} style={{ display: "flex", alignItems: "center", gap: 6, fontSize: 12.5, padding: "4px 10px 4px 4px", borderRadius: 20, background: "rgba(196,121,31,0.08)", color: GOLD, border: "1px solid rgba(196,121,31,0.3)" }}>
